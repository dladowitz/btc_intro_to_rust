#![allow(unused_variables)]
fn main() {
    result_test();
    let version = read_version("");
    println!("Version: {}!", version);
}

fn result_test() {
    type MyResult<'a> = Result<i32, &'a str>;
    
    let x: MyResult = Ok(-3);
    assert_eq!(x.is_ok(), true);
    println!("result: {}", x.unwrap());

    let y: MyResult = Err("Some error message");
    assert_eq!(y.is_ok(), false);
    println!("result: {}", y.unwrap_err());
}

fn read_version(transaction_hex: &str) -> u32 {
    // Decode the hex string to bytes
    let bytes = hex::decode(transaction_hex).expect("Failed to decode hex string");
    
    // Print the first 4 bytes
    if bytes.len() >= 4 {
        println!("First 4 bytes: {:02x?}", &bytes[..4]);
        
        // Parse the version number from the first 4 bytes (little-endian)
        let version = u32::from_le_bytes([bytes[0], bytes[1], bytes[2], bytes[3]]);
        println!("Parsed version: {}", version);
        
        return version;
    } else {
        println!("Transaction too short, only {} bytes", bytes.len());
        return 0;
    }
}

// Transaction on mempool.space
// https://mempool.space/testnet/tx/3c1804567a336c3944e30b3c2593970bfcbf5b15a40f4fc6b626a360ee0507f2